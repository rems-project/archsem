...
let rec string_of_regval_gen : RegValGen.t -> string = function
  ...
  | Array vs ->
      Printf.sprintf "[%s]"
        (String.concat ", " (List.map string_of_regval_gen vs))
  | Struct fields ->
      Printf.sprintf "{ %s }"
        (String.concat ", "
           (List.map
    ...
    (fun (tid, reqs) ->
      ...
      List.for_all
        (fun (reg, req) ->
          ...
          | None ->
              failwith
                ("Outcome checker couldn't find register: " ^ Reg.to_string reg)
          | Some rv -> (
...
let run_executions model init fuel term outcomes =
  ...
  let observable =
    List.filter_map
      (function Litmus_parser.Observable c -> Some c | _ -> None)
      outcomes
  in
  let unobservable =
    List.filter_map
      (function Litmus_parser.Unobservable c -> Some c | _ -> None)
      outcomes
  in
  let errors =
    List.filter_map
  ...
  in
  List.iter
    (fun e -> msg (Printf.sprintf "%s[Error]%s %s" Terminal.red Terminal.reset e))
    errors;
  ...
  let observable_ok =
    List.for_all
      (fun cond ->
        let matched =
          List.exists (fun fs -> check_outcome fs cond) final_states
        in
        if (not matched) && final_states <> [] then
          msg
            (Printf.sprintf "%sObservable not found%s: %s" Terminal.red
    ...
    List.for_all Fun.id
      (List.mapi
         (fun i fs ->
           ...
           | Some cond ->
               msg
                 (Printf.sprintf "%sUnobservable found%s in execution %d: %s"
...
let run_litmus_test model filename =
  ...
  if not (Sys.file_exists filename) then (
    ...
    try
      ...
      let fuel =
        Otoml.find_opt toml Litmus_parser.get_int ["fuel"]
