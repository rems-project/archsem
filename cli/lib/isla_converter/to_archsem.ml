      ...
      (match List.assoc_opt "assertion" t with
       | Some (Otoml.TomlString s) ->
           ...
           if s = "true" || s = "" then
             ...
             let is_observable, parsed = Outcomes.parse_assertion s (expect = "sat") in
             let prefix = if is_observable then "observable" else "unobservable" in
             if parsed = [] then
               ...
               List.iter (fun (outcome: Outcomes.outcome) ->
                 ...
                 Hashtbl.iter (fun tid regs ->
                   let content = String.concat ", " (List.map (fun (r, v, is_eq) ->
                     if is_eq then Printf.sprintf "%s = %d" r v
                     else Printf.sprintf "%s = { op = \"ne\", val = %d }" r v
                   ) (List.rev regs)) in
