...
(** Compute all termination PCs for a thread based on control flow.
    Returns a list of possible termination addresses:
    - Main code end (for normal execution or ERET return)
    - Handler end (for handlers without ERET that don't return) *)
let compute_term_pcs tid va size sections =
  let main_code_end = Z.add va size in
  let handler_pattern = "thread" ^ tid in

  (* Find all handlers for this thread that don't end with ERET *)
  let handler_ends = List.filter_map (fun s ->
    if String.length s.Input_parser.sec_name >= String.length handler_pattern &&
    ...
    then Some (Z.add s.Input_parser.sec_addr s.Input_parser.sec_size)
    else None
  ) sections in

  (* Always include main code end, plus any handler ends for non-ERET handlers *)
  main_code_end :: handler_ends

(** Emit termination condition for a thread.
    If multiple termination PCs are possible, emits an array format. *)
let emit_term_cond out tid va size sections =
  let term_pcs = compute_term_pcs tid va size sections in
  match term_pcs with
  | [single_pc] ->
      (* Single PC - use simple format *)
      Printf.fprintf out "[[termCond]] # %s\n_PC = 0x%s\n\n" tid (Z.format "%x" single_pc)
  | multiple_pcs ->
      (* Multiple PCs - use array format *)
      let pc_strs = List.map (fun pc -> "0x" ^ Z.format "%x" pc) multiple_pcs in
      Printf.fprintf out "[[termCond]] # %s\n_PC = [%s]\n\n" tid (String.concat ", " pc_strs)
            ...
            List.iter (fun (outcome : Assertion_parser.outcome) ->
              ...
              let mem_parts = List.filter_map (fun (symbol, value, is_eq) ->
                ...
                | Some addr ->
                  ...
                  Some (Printf.sprintf "{ addr = 0x%s, value = 0x%x, size = 8%s }"
                    (Z.format "%x" addr) value op_str)
                | None ->
