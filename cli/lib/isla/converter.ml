...
let rec clause_of_conj = function
  | Assertion.Atom atom -> [atom]
  | Assertion.And (lhs, rhs) -> clause_of_conj lhs @ clause_of_conj rhs
  | Assertion.True -> []
  | Assertion.False -> failwith "assertion: false is not a valid conjunction"
  | Assertion.Or _ | Assertion.Not _ ->
    failwith "assertion: expected normalized conjunction"

let rec clauses_of_dnf_expr = function
  | Assertion.False -> []
  | Assertion.True -> [[]]
  | Assertion.Or (lhs, rhs) -> clauses_of_dnf_expr lhs @ clauses_of_dnf_expr rhs
  | expr -> [clause_of_conj expr]

let to_final_conds ~expect_is_sat ~resolve_sym ~memory_size assertion =
  let is_observable, dnf =
    match assertion with
    | Assertion.Not e -> (not expect_is_sat, clauses_of_dnf_expr e)
    | e -> (expect_is_sat, clauses_of_dnf_expr e)
  in
  List.filter_map
    (fun conj ->
      if conj = [] then
  ...
  let syms, encoded_threads =
    List.fold_left_map
      (fun syms (thread : Ir.thread) ->
        ...
        let syms, code_addr = Symbols.alloc_code syms (Bytes.length enc) in
        (syms, (thread, enc, code_addr)))
      syms ir.threads
  in
