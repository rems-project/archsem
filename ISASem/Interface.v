...
(** For explicit memory accesses, this determines if the access has some kind of
...
(** For explicit memory accesses, this determines if the access has
    extra release-acquire semantics and which ones. *)
...
(** The access kind defined the type of access and thus the synchronisation
...
(** The architecture parameter that must be provided to the interface *)
Module Type Arch.

  (** The type of register, most likely string, but may be more fancy *)
  Parameter reg : Type.

  (** The type of each register, often `bv 64` but it may be smaller of be a
      boolean *)
  ...
  (** Physical addresses type. Since models are expected to be architecture
      specific, there is no generic way to extract a bitvector from it*)
  Parameter pa : Type.

  (** Translation summary *)
  Parameter translation : Type.

  (** Abort description. This represent physical memory abort on memory
...
Module Interface (A : Arch).
  ...
  Module Deps.
    Record t :=
      make
        {
          (** The list of register the effect depends on. *)
          regs : list A.reg;
  ...
  | RegRead (reg : A.reg) (direct : bool) : outcome (A.reg_type reg)
    ...
    (** The direct or indirect flag is to specify how much coherence is required
        ...
        Generally writing the PC introduce no dependency because control
  ...
  (** An execution trace for a single instruction.
      ...
      Is the option is Some, it means a GenericFail *)
  Definition instr_trace : Type := list event * option string.

  (** Definition of a trace matching a semantic *)
