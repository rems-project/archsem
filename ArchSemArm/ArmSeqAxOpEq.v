...
From ASCommon Require Import Exec FMon Common GRel.
...
(** * Lemmas about physical address arithmetic *)

(** Addresses use modular arithmetic. Hence memory accesses need to be limited
    to a maximum size to work correctly *)
...
Proof.
  ...
  destruct bv_is_wf
    as [?%Is_true_eq_true%Reflect.Z_leb_le ?%Is_true_eq_true%Reflect.Z_ltb_lt].
  assert (0 < z < bv_modulus 56)%Z by (unfold bv_modulus in *; lia); clear H.
  assert ((bv_unsigned + z < bv_modulus 56)%Z ∨
    (bv_modulus 56 ≤ bv_unsigned + z)%Z) as [|] by lia.
  { setoid_rewrite Z.mod_small at 1 2; try lia. }
  assert ((bv_unsigned + z)%Z =
    ((bv_unsigned + z - bv_modulus 56) + 1 * bv_modulus 56)%Z) as -> by lia.
...
Proof.
  ...
  - unfold SA.pa, mword, pa_addZ, max_mem_acc_size in *; cbn in *.
    ...
    destruct bv_is_wf
      as [?%Is_true_eq_true%Reflect.Z_leb_le ?%Is_true_eq_true%Reflect.Z_ltb_lt].
    assert (∀ x, (bv_unsigned + x)%Z =
      ((bv_unsigned + x - bv_modulus 56) + 1 * bv_modulus 56)%Z) by lia.
    assert ((bv_unsigned + x < bv_modulus 56)%Z ∨
      (bv_modulus 56 ≤ bv_unsigned + x)%Z) as [|] by lia;
    assert ((bv_unsigned + y < bv_modulus 56)%Z ∨
      (bv_modulus 56 ≤ bv_unsigned + y)%Z) as [|] by lia.
...
Lemma pa_addZ_exists_offset pa1 pa2 x y :
  ...
  ∃ o, (0 ≤ o)%Z ∧
    (pa_addZ pa1 o = pa2 ∧ (o ≤ x)%Z ∨ pa_addZ pa2 o = pa1 ∧ (o ≤ y)%Z).
Proof.
  cdestruct |- ***.
...
(** The size of memory accesses cannot be zero or greater than
    [max_mem_acc_size] to ensure properties in modular arithmetic  *)
...
(** An inductive predicate to state properties about all effects issued
    by a given free monad *)
...
(** The analogous predicate for choice monads *)
...
(** Memory access issued must have legal size as defined in [legal_memory_acc] *)
Definition isem_mem_acc_legal_length : iMon () → Prop :=
  cMon_call_inv legal_memory_acc.

(** * Basic automation instances used by [cdestruct] and [lookup_unfold] etc. *)
...
(** * Monotonic relations on events in traces *)

(** The acylicity argument in the refinement proof makes crucial use of the fact
    that the given relatons are monotone w.r.t. the full instruction order.
    That is, for any two event IDs related in a (strictly) monotone relationship,
    the first ID appears before the second ID. *)

Definition rel_strictly_monotone (rel : grel EID.t) : Prop :=
  ∀ '(x,y) ∈ rel, EID.full_po_lt x y.

(** This definition allows equality in addition to monotonicity *)
...
(** * Monad interpretation invariant induction *)

(** The following lemmas are specialised induction schemes to prove invariants
    maintained by a handler when interpreting a free/choice monad *)
...
(** This scheme allows to make use of properties guaranteed by the monad,
    e.g. [legal_memory_acc]  *)
...
Lemma finterp_state_mon_inv_induct `{Effect Eff} {St E A}
    (handler : fHandler Eff (Exec.t St E)) (mon : fMon Eff A)
    (I : fMon Eff A → St → Prop) (initSt : St) :
  I mon initSt →
  (∀ st call k, I (Next call k) st → ∀ st' ret, (st',ret) ∈ handler call st → I (k ret) st') →
  ∀ st' ret, (st',ret) ∈ FMon.finterp handler mon initSt → I (Ret ret) st'.
Proof.
  intros Hinit Hstep.
  induction mon as [|? ? IH] in initSt, Hinit |- *;
  cdestruct initSt |- *** #CDestrMatch.
  set_unfold.
  eapply IH.
  1: eapply Hstep.
  all: eauto.
...
Lemma cinterp_state_mon_inv_induct `{Effect Eff} {St E A}
    (handler : fHandler Eff (Exec.t St E)) (mon : cMon Eff A)
    (I : cMon Eff A → St → Prop) (initSt : St) :
  I mon initSt →
  (∀ st call k, I (Nextl call k) st → ∀ st' (ret : eff_ret call), (st',ret) ∈ handler call st → I (k (ret)) st') →
  (∀ st c k, I (Nextr c k) st → ∀ (ret : eff_ret c), I (k ret) st) →
  ∀ st' ret, (st',ret) ∈ FMon.cinterp handler mon initSt → I (Ret ret) st'.
Proof.
  intros Hinit HIpreserveCall HIpreserveChoice st' ret Hst'.
  eapply finterp_state_mon_inv_induct; [ eassumption | | eassumption].
  clear initSt Hinit st' Hst'.
  cdestruct |- *** as st eff  #CDestrMatch #CDestrSplitGoal;
  try destruct eff; intros.
  1: eapply (HIpreserveCall _ eff); eauto.
  1: eapply (HIpreserveChoice _); eauto.
Qed.

(** * Definitions and lemmas about lists *)
...
Lemma enumerate_imap {A} :
  enumerate (A := A) = imap pair.
Proof.
...
(** * Definitions and lemmas about (unicore) traces *)

(** Returns the first event in a given trace that satisfies the given predicate *)
...
(** As the traces generated by the operational model start at the head with
    the latest event, in contrast to candiadte pre-executions, it is
    necessary to reverse traces generated by the operational model *)
...
(** Returns the last event in a given trace that satisfies the given predicate *)
...
(** This definition is a stonger version of [trace_find_last] which allows the
    predicate to also make a statement about the index (event ID) of events
    in the given trace. This is needed as we, for example, commonly search for
    an event that appears before some other event. *)
Definition trace_find_last_indexed (P : EID.t → iEvent → Prop)
    `{∀ eid x, Decision (P eid x)} (itrs : list (iTrace ()))
    : option (EID.t * FMon.fEvent outcome) :=
  if
    find_last_indexed
      (λ iid '(itr,_),
        is_Some (find_last_indexed (λ ieid, P (EID.make 0 iid ieid None)) itr))
      itrs
  is Some (iid, itr)
  then
    (if find_last_indexed (λ ieid, P (EID.make 0 iid ieid None)) itr.1
    is (Some (ieid, ev))
    then Some (EID.make 0 iid ieid None, ev)
    else None)
  else None.

(** Append a single event to the end of a trace *)
Definition trace_snoc (ev : FMon.fEvent outcome) (itrs : list (iTrace ()))
    : list (iTrace ()) :=
...
(** This functions returns the event ID of succeeds the event ID of the
    last event in the given trace. In other words, this will be the event ID
    of an event that was appended to the end of the trace *)
...
(** As we only have one thread, we define a specialised [Lookup] instance
    that cuts out the unneeded [lookup_total] of the thead trace boilerplate *)
...
Lemma trace_find_last_indexed_spec (P : EID.t → iEvent → Prop)
    `{∀ eid x, Decision (P eid x)} (tr : list (iTrace ())) ev eid :
...
Proof.
  ...
  - enough (¬ EID.full_po_lt
    ...
    1: opose proof (EID.full_po_lt_connex eid'
      {| EID.tid := 0; EID.iid := n; EID.ieid := n1; EID.byte := None |} _ _ _)
      as Heid.
    ...
    1: done.
  - opose proof
      (H11 {| EID.tid := 0; EID.iid := n; EID.ieid := n1; EID.byte := None |}
        _ _ _).
    1: cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal.
...
Lemma trace_last_exists_indexed `{∀ eid ev, Decision (P eid ev)}
    (tr : list (iTrace ())) :
  ∀ eid ev, tr !! eid = Some ev → P eid ev →
  is_Some (trace_find_last_indexed P tr).
Proof.
...
Lemma trace_last_event_indexed P
    `{∀ (ev : iEvent) (eid : EID.t), Decision (P eid ev)}
...
(** * Auxiliary bitvector and memory lemmas *)
...
Proof.
  ...
  elim l; cdestruct pa' |- ***.
  erewrite (H (N.pred size) (pa_addZ pa0 1)
    (set (lookup pa0 ∘ mem) (λ _ : option (bv 8), Some a) st)); eauto.
  3: lia.
...
Lemma write_mem_seq_state_app pa l st st' x :
  ...
  (st', ()) ∈ write_mem_seq_state pa (l ++ [x]) st →
  (st', ()) ∈ write_mem_seq_state pa l
    (setv ((.!! pa_addN pa (N.of_nat (length l))) ∘ mem) (Some x) st).
Proof.
...
Proof.
  ...
  - unfold lookup at 2, list_lookupN.
    ...
    cdestruct |- *** #CDestrMatch #CDestrEqOpt;
    first by rewrite lookup_unfold in *.
...
Proof.
  ...
  change (itrs st)
    with (itrs (set (lookup pa0 ∘ mem) (λ _ : option (bv 8), Some a) st)).
  eapply IHbytes, H.
Qed.
...
Proof.
  induction bytes; cdestruct |- ***.
  change (regs st)
    with (regs (set (lookup pa0 ∘ mem) (λ _ : option (bv 8), Some a) st)).
  eapply IHbytes, H.
Qed.

Section RefinementProof.
(** From here we prove direct lemmas about the actual operational model.
    The model requires a whitelist of registers that can be written to,
    which we add to the context. *)
Context (regs_whitelist : gset reg).

Lemma sequential_model_outcome_same_itrs st st' call r :
  (st', r) ∈ sequential_model_outcome (Some regs_whitelist) call st →
   itrs st' = itrs st.
Proof.
  destruct call;
  cdestruct |- *** #CDestrMatch;
  try naive_solver.
  all: do 2 unfold mthrow, Exec.throw_inst, Exec.res_throw_inst in *;
    try set_solver.
...
Lemma write_mem_seq_state_same_initSt bytes :
  ∀ st st' pa, (st', ()) ∈ write_mem_seq_state pa bytes st →
  initSt st' = initSt st.
Proof.
  induction bytes; cdestruct |- ***.
  change (initSt st)
    with (initSt (set (lookup pa0 ∘ mem) (λ _ : option (bv 8), Some a) st)).
  eapply IHbytes, H.
Qed.
...
(** For the refinement proof we compute a (partial) candidate state for a
    given operational trace by updating the necessary relations for each
    event by folding over the trace. *)

(** The following record represents a (partial) candidate state. Besides
    the expected relatons found in a candidate execution, we also maintain two
    maps that keep track of the event EID that has most recently written to a
    physical address or register respectively. This is used to update the
    [reads_from], [coherence], and [register_reads_from].

    Additionally, for each physical address we maintain a set of event IDs that
    have written to this address in the past. This is needed as coherence
    relates *all* events that write to a certain physical address. *)
...
(** An empty partial candidate state, which is initially used.  *)
#[local] Notation "cd∅" := (Build_partial_cd_state ∅ ∅ ∅ ∅ ∅ ∅).

#[global] Instance eta_partial_cd_state : Settable partial_cd_state :=
  settable! Build_partial_cd_state
    <pa_write_map;reg_write_map;pa_writes_set_map;rf_acc;rrf_acc;co_acc>.

(** The following functions define the update logic of the partial candidate
    state for a given memory/register write/read event ID that is being
    appended at the end of the current trace *)
Definition mem_write_upd_partial_cd_state (pa : pa) (weid : EID.t)
    (st : partial_cd_state) : partial_cd_state :=
  let oprev_weid_set := st.(pa_writes_set_map) !! pa in
  (if oprev_weid_set is Some prev_weids
  then
    st
    |> set co_acc
        (fold_left (λ acc prev_weid, {[(prev_weid, weid)]} ∪ acc)
          (elements prev_weids))
...
Definition mem_read_upd_partial_cd_state (pa : pa) (reid : EID.t)
    (st : partial_cd_state) : partial_cd_state :=
...
Definition reg_write_upd_partial_cd_state (reg : reg) (weid : EID.t)
    : partial_cd_state → partial_cd_state :=
  setv (lookup reg ∘ reg_write_map) (Some weid).

Definition reg_read_upd_partial_cd_state (reg : reg) (reid : EID.t)
    (st : partial_cd_state) : partial_cd_state :=
...
(** The top-level function that combines the previous definitions *)
...
(** * Automation instances for the partial candidate state construction *)

(** As many core properties of our proof rely on these definitions, we have the
    following automation boilerplate *)
...
Proof using regs_whitelist.
  ...
  - fold_left_inv_pose
      (λ co_acc prev_weids, (eid1,eid2) ∈ co_acc ∨ eid1 ∈ prev_weids).
...
Lemma eq_some_unfold_memw_upd_pcdst_pa_w_map cdst pa pa' weid weid' P :
  ...
  EqSomeUnfold
    ((mem_write_upd_partial_cd_state pa' weid' cdst).(pa_write_map) !! pa)
...
Lemma eq_some_unfold_cdst_memw_upd_pa_ws_set_map cdst pa pa' weid ws
    (P : gset EID.t → Prop) Q :
...
(** The actual construction function that folds over the events of the
    Candidate in order, updating the partial candidate state in the meantime *)
Definition construct_cd_for_pe (pe : Candidate.pre Candidate.NMS 1)
    : partial_cd_state → partial_cd_state :=
  fold_left update_partial_cd_state_for_eid_ev (Candidate.event_list pe).

(** Constructs a one-threaded pre-execution and does not automatically
    unfold to avoid cluttering proof goals *)
...
Notation "(seq_state_to_pe seq_st )" :=
  (build_pre_exec seq_st.(initSt) seq_st.(itrs)) (seq_st at next level).

(** Computing a candidate state from partial candidate and a pre-execution *)
Definition partial_cd_state_to_cd (cdst : partial_cd_state)
    (pe : Candidate.pre Candidate.NMS 1) : Candidate.t Candidate.NMS 1 :=
...
Notation "(seq_state_to_cd seq_st )" :=
  (partial_cd_state_to_cd (seq_state_to_partial_cd_state seq_st)
    (seq_state_to_pe seq_st)) (seq_st at next level).
...
(** This Lemma when provided to [cdestruct] splits an event id lookup in a trace
    that was extended by one event, into two cases:
    the event is in the original trace, or it is the appended event itself.
    This is a core lemma used in most invariant proofs. *)
...
(** * Invariant preservation over traces *)

(** This states that the sequential model preserves an invariant [I'] over a
    single outcome interpretation. The initial state may have stronger
    properties (i.e. [I]). *)
Definition seq_model_outcome_invariant_preserved (I I' : seq_state → Prop)
    : Prop :=
  ∀ (seq_st : seq_state),
  I seq_st →
  ∀ (call : outcome) ret seq_st_succ,
  (seq_st_succ, ret) ∈
    sequential_model_outcome (Some regs_whitelist) call seq_st →
  I' (set itrs (trace_cons (call &→ ret)) seq_st_succ).

(** Invariant preservation with further assumptions about the legality
    of memory accesses and non-mixed-sizeness of the trace.  *)
  Definition seq_model_outcome_invariant_preserved_mem_assumptions
      (I I' : seq_state → Prop) : Prop :=
  ∀ (seq_st : seq_state),
  I seq_st →
  ∀ (call : outcome),
...
(** Destructing lemma for the computation of a partial candidate for
    an appended event  *)
...
Lemma regt_of_bv64_regt_to_bv64 {r} (x : reg_type r) :
  regt_of_bv64 (regt_to_bv64 x) = x.
Proof.
  unfold regt_of_bv64, regt_to_bv64.
  depelim r.
  by depelim r.
Qed.
    ...
    + eexists _, _, (_, _); cdestruct |- *** #CDestrSplitGoal #CDestrEqOpt.
      ...
      * rewrite Hsame_itrs in *.
        ...
        { eapply H_reg_map in H3.
          ...
          rewrite regt_of_bv64_regt_to_bv64.
...
(* The operational model preserves the full invariant record predicate. *)
...
Lemma is_overlapping_equal_events {et nmth} (pe pe' : Candidate.pre et nmth) :
  Candidate.event_list pe = Candidate.event_list pe' →
  ∀ eid1 eid2,
    Candidate.is_overlapping pe eid1 eid2 ↔ Candidate.is_overlapping pe' eid1 eid2.
Proof.
...
Lemma seqmodel_successor_nms_implies_start_nms fuel isem final_st start_st fs :
  ...
  (final_st, fs) ∈
    sequential_model_seqmon_final (Some regs_whitelist) fuel isem start_st →
  seq_state_is_nms start_st.
Proof.
  unfold sequential_model_seqmon_final.
  set_unfold.
  cdestruct |- *** as #CDestrMatch.
  intro final_nms.
  ...
  1: intros succ_nms.
  2: intros final_nms.
  all: intros fuel IH start_st.
  2: intros initfs.
  all: intros succ_st H_succ_exec.
  1: intros -> H_finalize.
  2: intros _ H_final_exec H_finalize.
  all: cycle 1.
  1: opose proof (IH _ _ H_final_exec _) as succ_nms; eauto.
...
Lemma sequential_model_outcome_logged_succ_tr st st' call ret :
  (st', ret) ∈ sequential_model_outcome_logged (Some regs_whitelist) call st →
  st'.(itrs) = trace_cons (call &→ ret) (itrs st).
Proof.
  unfold sequential_model_outcome_logged, fHandler_logger.
  cdestruct st' |- *** #CDestrEqOpt.
  by erewrite sequential_model_outcome_same_itrs.
Qed.

Lemma cMon_call_inv_True {A} (mon : iMon A) :
  cMon_call_inv (λ _ : outcome, True) mon.
Proof.
  induction mon as [|[|]].
  all: by constructor.
Qed.
...
Proof.
  induction isem as [|[|[]]].
  all: cdestruct |- ***.
  1,2: eexists; cdestruct |- *** #CDestrSplitGoal; eauto.
  ospecialize (H _ _); eauto.
  cdestruct H.
  eexists x.
  destruct n.
  all: depelim x0.
...
Proof.
  revert st.
  induction isem; cdestruct st', ret |- ***.
  unfold sequential_model_outcome_logged, fHandler_logger in H0.
  destruct call; deintros; cdestruct |- ***.
  2: { by eapply H. }
...
Lemma sequential_model_seqmon_trace_ind isem (I : iTrace () → Prop) fuel
  ...
  ∀ itr, itr ∈ seqst'.(itrs) → I itr.
Proof.
  intros ??.
  revert dependent seqst seqst' st.
  elim fuel; cdestruct |- *** #CDestrMatch.
  1: naive_solver.
  - cdestruct l, f0 #CDestrSplitGoal.
    1: by eapply H0 in H2.
    erewrite <- cinterp_seqmodel_trace in H4; eauto.
    naive_solver.
  - opose proof (H0 _ _ _ H2).
    eapply H.
    2,3: eauto.
    cdestruct l, f |- *** #CDestrSplitGoal.
    erewrite <- cinterp_seqmodel_trace in H7; eauto.
    naive_solver.
Qed.

Lemma seq_model_ISA_match isem initSt seqst st fuel :
  (seqst, st) ∈ sequential_model_seqmon (Some regs_whitelist) fuel isem
    {| initSt := initSt; regs := ∅; mem := ∅; itrs := [] |} →
  Candidate.ISA_match (seq_state_to_pe seqst) isem.
Proof.
  unfold build_pre_exec, Candidate.ISA_match.
  cdestruct |- *** as ????.
  rewrite lookup_total_unfold.
  generalize (l,f); clear l f; setoid_rewrite trace_rev_elem_of_P_equiv.
  eapply sequential_model_seqmon_trace_ind; eauto.
  1: set_solver.
  clear.
  cdestruct |- ***.
...
Proof. induction l in x |- *; naive_solver. Qed.

Lemma fold_left_fmap {A B C} (f : A → B → A) (x : A) (l : list C) (g : C → B) :
	fold_left f (g <$> l) x = fold_left (λ (a : A) (b : C), f a (g b)) l x.
Proof. rewrite ?fold_left_foldl. apply foldl_fmap. Qed.

Lemma fold_left_mjoin {A B} f (l : list (list B)) (a : A) :
  fold_left (λ acc l, fold_left f l acc) l a = fold_left f (mjoin l) a.
Proof.
  revert a.
...
Lemma Cand_events_event_list (pe : Candidate.pre Candidate.NMS 1) :
  mjoin (pe.(Candidate.events) !!! 0%fin).*1 = (Candidate.event_list pe).*2.
Proof.
  destruct pe.
  do 2 depelim events.
  ...
  cdestruct |- ***.
  clear init.
  assert (∀ B, @pair nat B = @pair nat B ∘ (Nat.iter 0 S)) as Hpairiter
    by (intro; apply functional_extensionality_dep; done).
  setoid_rewrite Hpairiter.
  rewrite (Hpairiter iEvent).
  generalize 0 at 1 as m; generalize 0 at 2 as n.
  induction h; try done.
  cbn.
  destruct a.
  assert (∀ n, Nat.iter n S ∘ S = Nat.iter (S n) S)
  by (intro; apply functional_extensionality; intros; by rewrite Nat.iter_succ_r).
  intros.
  rewrite Combinators.compose_assoc, (H n).
  rewrite (IHh (S n) m), ?app_nil_r, ?bind_app.
  cbn.
  rewrite fmap_app.
  f_equal.
  revert m; induction l; try done; cdestruct |- *** #CDestrSplitGoal.
  rewrite (IHl (S m)) at 1.
  by rewrite Combinators.compose_assoc, (H m).
Qed.
...
Proof.
  ...
  rewrite ?enumerate_imap.
  cbn.
  rewrite ?imap_app, ?bind_app, ?app_nil_r.
  done.
Qed.
...
Proof.
  ...
  1: by cbv.
  destruct (decide (itr = [])) as [->|Hend%exists_last].
  2: { destruct Hend as [? [ev ->]]; eauto. }
  intros ?.
  rewrite event_list_snoc_empty in *.
  ospecialize (IHitrs _); eauto.
Qed.

Lemma trace_snoc_unfolded itrs itr trend ev :
  trace_snoc ev (itrs ++ [(itr,trend)]) = itrs ++ [(itr ++ [ev],trend)].
Proof.
  unfold trace_snoc, unsnoc_total.
  by rewrite unsnoc_snoc.
Qed.

Lemma event_list_ordered (pe : Candidate.pre Candidate.NMS 1) front back :
  Candidate.event_list pe = front ++ back →
  ∀ eid ev eid' ev', (eid,ev) ∈ front → (eid',ev') ∈ back →
  EID.full_po_lt eid eid'.
Proof.
  destruct pe.
  do 2 depelim events.
  revert front h.
  induction back using rev_ind; cdestruct |- ***.
  opose proof (event_list_snoc_not_empty init h0 _).
  1: rewrite H; destruct front, back; set_solver.
  cdestruct H2 |- ***.
  rewrite H2 in *; clear H2.
  rewrite <- trace_snoc_unfolded in *.
  ...
  rewrite lookup_unfold in H1; done.
Qed.

Lemma seq_partition_ordered n1 n2 l1 l2 :
  seq n1 n2 = l1 ++ l2 → ∀ x ∈ l1, ∀ y ∈ l2, x < y.
Proof.
  induction l2 in n1, n2, l2 |- * using rev_ind; cdestruct |- *** #CDestrSplitGoal.
  - destruct n2; first (destruct l1, l2; discriminate).
    rewrite seq_end, app_assoc in H.
    apply app_inj_tail in H; cdestruct H.
    eapply IHl2; eauto.
  - destruct n2; first (destruct l1, l2; discriminate).
    rewrite seq_end, app_assoc in H.
    apply app_inj_tail in H; cdestruct H.
    assert (x0 ∈ l1 ++ l2) by set_solver.
    rewrite <- H in *.
    set_unfold.
    lia.
Qed.

Lemma seq_partition_complete_smaller n1 n2 l1 l2 x :
  seq n1 n2 = l1 ++ x :: l2 → ∀ y, n1 ≤ y < x ↔ y ∈ l1.
Proof using regs_whitelist.
  induction l1 in n1, n2, l2 |- *; cdestruct |- ***.
  1: destruct n2; deintros; cdestruct |- *** #CDestrSplitGoal; lia.
  destruct n2; first done; deintros; cdestruct |- ***.
  set_unfold.
  setoid_rewrite <- IHl1; eauto.
  cdestruct |- *** #CDestrSplitGoal; try lia.
  assert (x ∈ l1 ++ x :: l2) by set_solver.
  rewrite <- H in *.
  set_solver.
Qed.

Lemma final_write_per_pa_seqmodel_correct st pa eid b :
  seq_inv_predicate st →
  seq_state_is_nms st →
  let cd := (seq_state_to_cd st) in
  Candidate.final_write_per_pa cd !! pa = Some (eid, b) ↔
  ∃ ev evpa size offset v,
    pa = (pa_addN evpa offset) ∧
    cd !! eid = Some ev ∧ is_mem_write ev ∧ get_pa ev = Some evpa ∧
    get_mem_value ev = Some v ∧ get_size ev = Some size ∧
    (offset < size)%N ∧ bv_get_byte 8 offset (bvn_val v) = b ∧
    ∀ eid' ev', cd !! eid' = Some ev' →
      is_mem_writeP
        (λ size wr, pa_in_range (WriteReq.pa wr) size pa) ev' →
        EID.full_po_lt eid' eid ∨ eid' = eid.
Proof.
  intros H_inv Hnms cd.
  enough (Candidate.final_write_per_pa cd !! pa = Some (eid, b) ↔
    ∃ ev evpa size offset v,
    pa = (pa_addN evpa offset) ∧
    cd !! eid = Some ev ∧ is_mem_write ev ∧ get_pa ev = Some evpa ∧
    get_mem_value ev = Some v ∧ get_size ev = Some size ∧
    (offset < size)%N ∧ bv_get_byte 8 offset (bvn_val v) = b ∧
    ∀ eid', (eid, eid') ∉ (Candidate.coherence cd)) as H.
  {
    setoid_rewrite H; clear H.
    split.
    - cdestruct |- *** #CDestrEqOpt.
      eexists _, _, _, _, _.
      unfold is_mem_writeP, is_mem_write_reqP.
      cdestruct |- *** #CDestrSplitGoal #CDestrEqOpt; eauto; try done.
      enough (¬ EID.full_po_lt eid eid').
      1: opose proof (EID.full_po_lt_connex eid eid' _ _ _ ).
      1-4: unfold lookup, lookup_ev_from_iTraces in *; deintros;
        cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal; naive_solver.
      intro.
      eapply (H3 eid').
      ...
      unfold is_mem_writeP, is_mem_write_reqP.
      cdestruct |- *** #CDestrSplitGoal #CDestrEqOpt; eauto; try done.
      all: destruct ev' as [[]]; try fast_done; deintros; cdestruct |- *** #CDestrMatch.
      ospecialize (Hnms (eid, eid') _).
      1: {
        unfold Candidate.overlapping, Candidate.is_overlapping,
        Candidate.mem_events, is_mem_event, is_mem_writeP.
        apply pa_in_range_spec in H5; cdestruct H5.
        do 5 (cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal #CDestrMatch;
          try eexists; try by right).
        apply pa_overlap_spec; eauto. }
      unfold Candidate.same_footprint, Candidate.same_pa in *.
      set_unfold in Hnms; cdestruct Hnms |- *** #CDestrEqOpt.
    - cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal; eexists _, _, _, _, _;
      cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal.
      intros Hco%(cdst_co_acc_inv _ H_inv); cdestruct Hco #CDestrEqOpt.
      unfold is_mem_write in *.
      ospecialize (H3 eid' _ _ _); cdestruct |- *** #CDestrEqOpt.
      1: eapply pa_in_range_spec; eexists x2; split; first sauto.
      1: ospecialize (Hnms (eid, eid') _).
      1: {
        unfold Candidate.overlapping, Candidate.is_overlapping, Candidate.mem_events,
          is_mem_event, is_mem_writeP.
        do 5 (cdestruct eid, eid' |- *** #CDestrEqOpt #CDestrSplitGoal #CDestrMatch;
          try eexists; try by right).
        eapply pa_overlap_spec.
        eexists 0%N, 0%N.
        opose proof (pcdst_mem_acc_legal _ H_inv eid' _ _); cdestruct |- *** #CDestrEqOpt; cbn in *.
        rewrite ?pa_addN_zero.
        cdestruct |- *** #CDestrSplitGoal.
        all: lia.
      }
      1: unfold Candidate.same_footprint, Candidate.same_size in *; deintros;
        cdestruct |- *** #CDestrEqOpt.
      cdestruct eid, H3 |- *** #CDestrSplitGoal.
  }
  unfold Candidate.final_write_per_pa.
  fold_left_inv_complete_pose
    (λ mmap unproc proc,
      ∀ pa (eid : EID.t) b, mmap !! pa = Some (eid, b) ↔
      ∃ ev evpa size offset val, (eid, ev) ∈ proc ∧ is_mem_write ev ∧
        get_pa ev = Some evpa ∧
        pa_addN evpa offset = pa ∧ get_size ev = Some size ∧ (offset < size)%N ∧
        get_mem_value ev = Some val ∧ bv_get_byte 8 offset (bvn_val val) = b ∧
        ∀ ev' (eid' : EID.t),
        (eid', ev') ∈ proc → (eid, eid') ∉ Candidate.coherence cd).
  1: split.
  all: cdestruct |- *** #CDestrEqOpt.
  - destruct (decide (is_mem_write f)).
    + destruct f as [[]]; unfold is_mem_writeP in *; deintros; cdestruct |- *** #CDestrMatch.
      fold_left_inv_complete_pose
        (λ mmap unproc proc, (∀ offset ∈ proc, (N.of_nat offset < n)%N) ∧
        ∀ pa,
          ∀ (eid : EID.t) b,
            mmap !! pa = Some (eid, b) ↔
            (∃ offset ∈ proc, pa = pa_addN (WriteReq.pa wr) (N.of_nat offset) ∧
              bv_get_byte 8 (N.of_nat offset) (WriteReq.value wr) = b ∧
              eid = t ∧
              ∀ '((eid', ev') : EID.t * iEvent) ∈ strt, ∀ n' wr', ev' = MemWrite n' wr' &→ inl true →
                (eid, eid') ∉ Candidate.coherence (seq_state_to_cd st))
            ∨ (a !! pa = Some (eid, b) ∧
              ∀ offset ∈ proc, ¬ pa_in_range (WriteReq.pa wr) (N.of_nat (S offset)) pa)).
      all: cdestruct |- *** #CDestrEqOpt.
      * cdestruct |- *** #CDestrSplitGoal #CDestrEqOpt #CDestrMatch.
        setoid_rewrite pa_in_range_spec; unfold not.
        1: right; cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal.
      * split.
        1: cdestruct |- *** #CDestrSplitGoal; last by eapply H5.
        1: enough (x ∈ seq 0 (N.to_nat n)) by (set_unfold; lia).
        1: set_solver.
        assert (a0 !! pa_addZ (WriteReq.pa wr) (Z.of_nat x) =
            a !! pa_addZ (WriteReq.pa wr) (Z.of_nat x)) as ->.
        {
          eapply option_eq; intros [].
          setoid_rewrite H6.
          opose proof (pcdst_mem_acc_legal _ H_inv t _ _); cdestruct |- *** #CDestrEqOpt; cbn in *.
          cdestruct |- *** #CDestrSplitGoal.
          - opose proof (seq_partition_ordered _ _ _ _ H4 x0 _ x _).
            all: cdestruct |- ***.
            1: by left.
            unfold pa_addN in *; eapply pa_addZ_lt_max_mem_acc_size_inj in H9; try lia.
          - right; cdestruct |- *** #CDestrSplitGoal.
            intros ?%pa_in_range_spec; deintros; cdestruct |- ***.
            unfold pa_addN in *; eapply pa_addZ_lt_max_mem_acc_size_inj in H11; try lia.
            2: { ospecialize (H5 offset _); try done. lia. }
            opose proof (seq_partition_ordered _ _ _ _ H4 offset _ x _); cdestruct |- ***.
            1: by left.
            lia.
        }
        split.
        {
          cdestruct |- *** #CDestrSplitGoal #CDestrMatch.
          1:{
            exfalso.
            eapply H1 in H7; cdestruct H7.
            opose proof (event_list_ordered _ _ _ H0 t0 (MemWrite x5 x6 &→ x7) t _ _ _);
            cdestruct |- ***.
            1: by left.
            apply (cdst_co_acc_inv _ H_inv) in e; cdestruct e #CDestrEqOpt.
          }
          {
            rewrite lookup_unfold in H8; deintros; cdestruct |- *** #CDestrMatch.
            1: left; eexists x; cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal; first by left.
            1: unfold pa_addN; f_equal; lia.
            1: intros Hco%(cdst_co_acc_inv _ H_inv).
            1: opose proof (event_list_ordered _ _ _ H0 t1 _ t _ _ _); cdestruct |- ***; eauto;
              deintros; cdestruct |- ***.
            apply H6 in H9; cdestruct H9 #CDestrEqOpt #CDestrSplitGoal.
            1: left; eexists; cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal; first by right.
            all: try done.
            1: intros Hco%(cdst_co_acc_inv _ H_inv).
            1: opose proof (event_list_ordered _ _ _ H0 t1 _ t _ _ _); cdestruct |- ***; eauto;
              deintros; cdestruct |- ***.
            right; split; try done.
            cdestruct |- *** #CDestrSplitGoal; last sauto.
            intros ?%pa_in_range_spec; deintros; cdestruct |- ***.
            opose proof (pcdst_mem_acc_legal _ H_inv t _ _); cdestruct |- *** #CDestrEqOpt; cbn in *.
            destruct (decide (x =  N.to_nat x0)) as [->|];
              first (eapply H8; unfold pa_addN; f_equal; by rewrite ?N_nat_Z).
            eapply (H10 (N.to_nat x0)).
            1: opose proof (seq_partition_complete_smaller _ _ _ _ _ H4 (N.to_nat x0)).
            1: set_unfold; eapply H13.
            1: lia.
            apply pa_in_range_spec; eexists; split; eauto; lia.
          }
          {
            rewrite lookup_unfold in H8; deintros; cdestruct |- *** #CDestrMatch.
            1: left; eexists x; cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal; first by left.
            1: unfold pa_addN; f_equal; lia.
            1: intros Hco%(cdst_co_acc_inv _ H_inv).
            1: opose proof (event_list_ordered _ _ _ H0 t0 _ t _ _ _); cdestruct |- ***; eauto;
              deintros; cdestruct |- ***.
            apply H6 in H9; cdestruct H9 #CDestrEqOpt #CDestrSplitGoal.
            1: left; eexists; cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal; first by right.
            all: try done.
        ...
        {
          unfold pa_addN in *; setoid_rewrite nat_N_Z; rewrite ?nat_N_Z in *.
          opose proof (pcdst_mem_acc_legal _ H_inv t _ _).
          1: cdestruct |- *** #CDestrEqOpt; cbn in *.
          unfold legal_memory_acc_ev in *; cbn in *.
          cdestruct H7 as Hlegal1 Hlegal2.
          deintros; cdestruct |- *** #CDestrSplitGoal #CDestrMatch #CDestrEqOpt.
          1,4,7: apply (cdst_co_acc_inv _ H_inv) in e; cdestruct e |- *** #CDestrEqOpt.
          1: eapply H1 in H8.
          2,3: eapply H1 in H9.
          1-3: cdestruct t0 |- ***; exfalso.
          1-3: opose proof (event_list_ordered _ _ _ H0 t0 _ t _ _ _); cdestruct |- ***; eauto.
          1-3: deintros; cdestruct |- ***.
          all: rewrite lookup_unfold; cdestruct |- *** #CDestrMatch #CDestrSplitGoal.
          1,3: opose proof (seq_partition_ordered _ _ _ _ H4 x0 _ x _);
            cdestruct |- ***; try by left.
          1,2: eapply pa_addZ_lt_max_mem_acc_size_inj in H10; try lia.
          1,2: eapply H6; left; eexists; cdestruct |- *** #CDestrSplitGoal; eauto.
          1,3: f_equal; lia.
          1,2: intros Hco%(cdst_co_acc_inv _ H_inv); cdestruct Hco |- *** #CDestrEqOpt.
          1-2: deintros; cdestruct |- ***; exfalso.
          1-2: opose proof (event_list_ordered _ _ _ H0 _ _ t _ _ _); cdestruct |- ***; eauto.
          1-2: deintros; cdestruct |- ***.
          1,2,4,5: exfalso; eapply (H8 x); first by left.
          1-4: eapply pa_in_range_spec; eexists (N.of_nat x); split; last lia.
          1-4: unfold pa_addN in *; by setoid_rewrite nat_N_Z.
          all: eapply H6; right; cdestruct |- *** #CDestrSplitGoal; eapply H8; set_solver.
        }
      * cbn in *.
        cdestruct H2.
        setoid_rewrite H3; clear H3.
        split; cdestruct |- *** #CDestrEqOpt.
        {
          cdestruct t, H3 |- *** #CDestrSplitGoal #CDestrEqOpt.
          1: eexists (MemWrite n wr &→ inl true), _, _, _, _;
          cdestruct |- *** #CDestrSplitGoal #CDestrEqOpt #CDestrMatch.
          1: by left.
          1: eapply H2; cdestruct |- ***; lia.
          1: intros Hco%(cdst_co_acc_inv _ H_inv); cdestruct Hco #CDestrEqOpt.
          ...
          cdestruct H3; eexists _, _, _, _, _; repeat apply conj.
          1: by right.
          all: eauto.
          ...
          1: sauto.
          unfold Candidate.same_footprint, Candidate.same_size in *.
          cdestruct t |- *** #CDestrEqOpt.
          lia.
        }
        {
          deintros; cdestruct |- *** #CDestrSplitGoal #CDestrEqOpt.
          - left; eexists (N.to_nat x2).
            cdestruct |- *** #CDestrSplitGoal.
            3,4: f_equal.
            1,2,3,4: lia.
            eapply H8.
          ...
          - right.
            ...
            + setoid_rewrite pa_in_range_spec.
              ...
              * ospecialize (Hnms (eid0, t) _).
                ...
                unfold Candidate.same_footprint, Candidate.same_pa in *.
                cdestruct Hnms |- *** #CDestrEqOpt.
              * opose proof (event_list_ordered _ _ _ H0 eid0 _ t _ _ _);
                cdestruct |- *** #CDestrEqOpt; eauto.
        }
    + enough (a !! pa0 = Some (eid0, b0)
      ↔ ∃ (ev : iEvent) (evpa : Arch.pa) (size offset : N) (val : bvn),
        (eid0, ev) ∈ (t, f) :: strt
        ∧ is_mem_write ev
          ∧ get_pa ev = Some evpa
            ∧ pa_addN evpa offset = pa0
              ∧ get_size ev = Some size
                ∧ (offset < size)%N
                  ∧ get_mem_value ev = Some val
                    ∧ @bv_get_byte 8 offset (bvn_n val) (bvn_val val) = b0
                    ∧ ∀ (ev' : iEvent) (eid' : EID.t),
                        (eid', ev') ∈ (t, f) :: strt
                      → (eid0, eid') ∉
                      co_acc (construct_cd_for_pe
                        (build_pre_exec (initSt st) (itrs st)) cd∅)).
      1:{
        opose proof (mem_writes_succeed _ H_inv f).
        unfold is_mem_writeP in *; destruct f as [[]]; try assumption.
        ospecialize (H3 _ _); cdestruct |- *** #CDestrEqOpt #CDestrMatch.
        1: eexists (t, _); cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal.
        deintros; by cdestruct |- *** #CDestrMatch.
      }
      setoid_rewrite H1; clear H1.
      unfold is_mem_writeP.
      split.
      all: cdestruct |- *** #CDestrEqOpt.
      all: eexists _, _, _, _, _.
      all: cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal; eauto.
      all: unfold is_mem_writeP in *.
  ...
  - cbn in *.
    ...
    intro.
    opose proof H4.
    apply (cdst_co_acc_inv _ H_inv) in H4.
    cdestruct H4 #CDestrEqOpt.
    eapply (H3 _ eid').
    all: cdestruct |- *** #CDestrEqOpt.
Qed.

Lemma final_reg_map_tid_seqmodel_correct st reg (v : reg_type reg) :
  seq_inv_predicate st →
  let cd := (seq_state_to_cd st) in
  (∃ ev eid,
    cd !! eid = Some ev ∧ is_reg_write ev ∧ get_reg ev = Some reg ∧
    get_reg_val ev = Some (existT reg v) ∧
    ∀ eid' ev', cd !! eid' = Some ev' → is_reg_write ev' → get_reg ev' = Some reg →
      EID.full_po_lt eid' eid ∨ eid' = eid) ∨
    ((¬ ∃ ev eid, cd !! eid = Some ev ∧ is_reg_write ev ∧ get_reg ev = Some reg) ∧
      v = (st.(initSt).(MState.regs) !!! 0%fin) reg) →
  Candidate.final_reg_map_tid cd 0%fin reg = v.
Proof.
  cdestruct |- *** #CDestrEqOpt.
  ...
  - destruct (decide (is_reg_write (fcall &→ fret))).
    2: {
      ...
      eexists x, (RegWrite reg x2 r0 &→ ()); cdestruct |- *** #CDestrSplitGoal.
      1: by right.
      1: unfold is_reg_writeP in *; naive_solver.
      by eapply H4.
    }
    unfold is_reg_writeP.
    destruct fcall; try done.
    cdestruct r, r0, H6 |- *** #CDestrMatch.
    2: {
      eexists x, (RegWrite reg x2 r &→ ()); cdestruct |- *** #CDestrSplitGoal.
      1: by right.
      1: unfold is_reg_writeP in *; naive_solver.
    }
    eexists t, (RegWrite reg0 racc regval &→ fret).
    cdestruct reg0 |- *** #CDestrSplitGoal.
    1,2: naive_solver.
    rewrite cons_middle in H2.
    eapply event_list_ordered in H2; eauto.
    1: instantiate (1 := ev').
    1: set_solver.
    eapply elem_of_app; left; sfirstorder.
  - rewrite cons_middle, app_assoc in H2.
    eapply event_list_ordered; eauto.
    eapply elem_of_app; right; sfirstorder.
  - rewrite cons_middle, app_assoc in H2.
    eapply event_list_ordered; eauto.
    instantiate (1 := i1).
    set_solver.
  - destruct fcall; try done.
    cdestruct reg0, H4 |- *** #CDestrMatch.
    eexists t, (RegWrite reg racc r &→ ()).
    cdestruct |- *** #CDestrSplitGoal.
    1,2: naive_solver.
    rewrite cons_middle in H2.
    eapply event_list_ordered in H2; eauto.
    1: instantiate (1 := RegWrite reg x0 x1 &→ ()).
    1: set_solver.
    eapply elem_of_app; left; sfirstorder.
  - unfold not; cdestruct |- ***.
    ospecialize (H3 (t0,i) _); eauto.
    unfold is_reg_writeP in *.
    cdestruct i, H3 |- *** #CDestrSplitGoal; naive_solver.
  - rewrite cons_middle, app_assoc in H2.
    eapply event_list_ordered; eauto.
    instantiate (1 := i).
    set_solver.
  - eapply event_list_ordered; eauto.
    1: instantiate (1 := i1); set_solver.
    instantiate (1 := i0); set_solver.
  - cdestruct H0, H1 |- *** #CDestrMatch #CDestrEqOpt #CDestrSplitGoal.
    + enough (x = x0) by naive_solver.
      ospecialize (H1 x (RegWrite reg x4 r &→ ()) _ _ _);
      cdestruct |- *** #CDestrEqOpt.
...
Lemma seq_model_cd_MState_final_match st :
  seq_inv_predicate st →
  seq_state_is_nms st →
  MState.finalize (seq_state_to_init st) =
  Candidate.cd_to_MState_final (seq_state_to_cd st) st.(initSt).(MState.termCond).
Proof.
  intros H_inv Hnms.
  unfold seq_state_to_init, Candidate.cd_to_MState_final, Candidate.cd_to_MState,
    seq_state_to_init_mem, seq_state_to_init_regs, Candidate.final_mem_map.
  do 3 f_equal; cbn.
  2: f_equal.
  all: apply functional_extensionality_dep.
  - cdestruct |- *** #CDestrMatch.
    + apply (seq_st_mem_map_consistent _ H_inv _) in H.
      cdestruct b, x, H |- ***.
      eapply final_write_per_pa_seqmodel_correct in H0; eauto.
      ...
      cbn in *.
      unfold Candidate.same_footprint, Candidate.same_pa in *.
      cdestruct b0, x9, x4, Hnms |- *** #CDestrEqOpt.
      rewrite H5 in *.
      eapply pa_addZ_lt_max_mem_acc_size_inj in H0.
      2: opose proof (pcdst_mem_acc_legal _ H_inv x1 _ _);
        deintros; cdestruct |- *** #CDestrEqOpt; lia.
      2: opose proof (pcdst_mem_acc_legal _ H_inv t _ _);
        deintros; cdestruct |- *** #CDestrEqOpt; lia.
    ...
    + exfalso; revert H0; eapply not_eq_None_Some.
      ...
      cdestruct H |- *** #CDestrEqOpt.
      eexists (x1, _).
      setoid_rewrite final_write_per_pa_seqmodel_correct; eauto.
      eexists (MemWrite x3 x4 &→ inl true), _, _, _, _.
      cdestruct x |- *** #CDestrEqOpt #CDestrSplitGoal.
      eapply H3.
      all: cdestruct x |- *** #CDestrEqOpt #CDestrSplitGoal.
    + exfalso; revert H; eapply not_eq_None_Some.
      apply final_write_per_pa_seqmodel_correct in H0; eauto.
      cdestruct b, x, H0 |- *** #CDestrEqOpt.
      eexists _.
      setoid_rewrite (seq_st_mem_map_consistent _ H_inv _).
      eexists _, t, _.
      unfold is_mem_writeP.
      cdestruct t, x |- *** #CDestrEqOpt #CDestrSplitGoal #CDestrMatch.
      eapply H1.
      all: cdestruct t, x |- *** #CDestrEqOpt #CDestrSplitGoal #CDestrMatch.
  - intros.
    eapply eq_sym.
    eapply final_reg_map_tid_seqmodel_correct; eauto.
    cdestruct |- *** #CDestrMatch.
    + left.
      eapply (seq_st_reg_map_consistent _ H_inv _) in H.
      cdestruct b, H |- *** #CDestrEqOpt.
      eexists _, x1.
      cdestruct |- *** #CDestrEqOpt #CDestrSplitGoal.
      1: by rewrite regt_of_bv64_regt_to_bv64.
      eapply H0; cdestruct |- *** #CDestrEqOpt.
    + right.
      unfold not.
      cdestruct |- *** #CDestrSplitGoal #CDestrEqOpt.
...
Lemma sequential_model_seqmon_init_always_from_seqst isem fuel st st' init :
  (st', init) ∈ sequential_model_seqmon (Some regs_whitelist) fuel isem st →
  seq_state_to_init st' = init.
Proof.
  revert st.
  induction fuel; cdestruct st', init |- *** #CDestrMatch.
  by eapply IHfuel.
Qed.

(** * The refinement theorem *)

(** This is the main result of this file. We assume
    - any memory access issued by the instruction semantics
      has size ≠ 0 and < 2 ^ 52
    - a sequential state that was computed by the operational model without error
    - no mixed-size memory accesses in the operational model trace

    and prove that the computed candidate execution that has the same trace
    is **consistent** and **well-formed**!

    The result follows directly from the sequential model maintaining the big
    invariant record defined above. *)
Theorem op_model_refines_ax_model fuel isem initSt :
  isem_mem_acc_legal_length isem →
  ∀ st fs, seq_state_is_nms st →
    (st, fs) ∈
      sequential_model_seqmon_final (Some regs_whitelist) fuel isem
      {| initSt := initSt; regs := ∅; mem := ∅; itrs := [] |} →
    let cd := (seq_state_to_cd st) in
      Candidate.ISA_match cd isem ∧ cd.(Candidate.pre_exec) = (seq_state_to_pe st) ∧
      Candidate.cd_to_MState_final cd st.(ArmSeqModel.initSt).(MState.termCond) = Some fs ∧
      consistent regs_whitelist cd ∧ Candidate.wf cd.
Proof.
  cdestruct |- *** as Hmemlength ?? Hnms Hexec.
  unfold sequential_model_seqmon_final in *.
  cdestruct st |- *** as ?? fsinit ? H_finalize #CDestrEqOpt #CDestrMatch.
  enough (Candidate.ISA_match (seq_state_to_pe st) isem ∧ seq_inv_predicate st) as [? Hinv].
  {
    erewrite <- (seq_model_cd_MState_final_match),
      sequential_model_seqmon_init_always_from_seqst; eauto.
    by destruct Hinv.
  }
  split.
  1: by eapply seq_model_ISA_match.
  pose proof (empty_seq_state_fulfills_seq_inv_predicate initSt).
  revert H H0.
  enough (∀ st',
    (st, fsinit) ∈ sequential_model_seqmon (Some regs_whitelist) fuel isem st' →
    seq_inv_predicate st' → seq_inv_predicate st) by naive_solver.
  revert dependent isem st initSt fsinit fs.
  elim fuel; clear fuel; cdestruct |- *** as fuel IH isem Hmemlength #CDestrMatch.
  1: cdestruct |- *** as
    one_inst_seqst initSt H_inst_cinterp fs H_finalize one_inst_seqst_nms init_seqst
    H_inv_init
    #CDestrEqOpt.
  2: intros final_seqst final_seqst_nms _ ?? H_finalize init_seqst one_inst_seqst H_inst_cinterp
    H_finalize_one_inst H_rest_exec H_inv_init.
  1: apply seq_inv_predicate_preserved_change_trace_end.
  all: cycle 1.
  1: eapply IH; eauto.
  1: apply seq_inv_predicate_preserved_change_trace_end.
  1: opose proof (seqmodel_successor_nms_implies_start_nms fuel isem final_seqst
    (set itrs
      (λ l : list (iTrace ()), ((hd FTNothing l).1, FTERet ()) :: tail l)
      one_inst_seqst)
    fs _ _)
    as one_inst_seqst_nms; eauto.
  1: unfold sequential_model_seqmon_final; cdestruct |- ***; eexists _, _;
    cdestruct |- *** #CDestrMatch #CDestrSplitGoal; eauto; set_unfold; naive_solver.
